/// This module provides structs for representing velocities as dimensionful quantities.
use std::ops::Add;
...
pub fn sum_with_scaled_other(
    base_velocity: &DimensionfulVector,
    velocity_to_scale: &DimensionfulVector,
    scaling_factor: f64,
) -> DimensionfulVector {
    DimensionfulVector {
        horizontal_component: HorizontalUnit(
            base_velocity.horizontal_component.0
                + (scaling_factor * velocity_to_scale.horizontal_component.0),
        ),
        vertical_component: VerticalUnit(
